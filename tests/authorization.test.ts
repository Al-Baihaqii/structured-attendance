...
test("MUSYRIF group query is limited to assigned groups", () => {
  ...
  assert.deepEqual(getAccessibleGroupsWhere(musyrif), { assignments: { some: { musyrifId: "musyrif-1", endedAt: null } }, sector: { mahalli: { cityId: "city-1" } } });
});

test("active Musyrif reads complete history but only edits current tenure", () => {
  ...
  assert.throws(() => assertSessionAccess(former, session));
  assert.equal(canViewGroup(former, g), false);
  assert.equal(canViewGroup(replacement, g), true);
  assert.throws(() => assertSessionAccess(former, session, "manage"));
  assert.equal(canEditSession(former, session), false);
  assert.doesNotThrow(() => assertSessionAccess(replacement, session));
  ...
  assert.throws(() => assertSessionAccess(replacement, { ...session, assignment: null }, "manage"));
