...
require.cache[prismaPath]!.exports = { prisma: { $transaction: async (fn: any, options: any) => {
  ...
  try {
    const result = await fn({
      ...
      attendanceSession: { create: async ({ data }: any) => {
        calls.push("session");
...
beforeEach(() => {
  ...
  group = { id: "g1", name: "Group", status: "ACTIVE", sectorId: "s1", sector: { mahalliId: "h1", mahalli: { cityId: "c1" } }, userGroups: [{ userId: "u1" }] };
...
for (const role of ["SUPER_ADMIN", "CITY_ADMIN", "MAHALLI_ADMIN", "SECTOR_ADMIN", "MUSYRIF"]) {
  for (const status of ["ACTIVE", "INACTIVE"]) {
    test(`${role} can create for ${status} group with matching audit log`, async () => {
      ...
      assert.deepEqual(calls, ["lock", "group", "session", "log"]);
    });
  }
...
test("concurrent duplicate meeting creates only one session and log", async () => {
  const responses = await Promise.all([call(), call()]);
  assert.deepEqual(responses.map(r => r.status).sort(), [201, 409]);
  assert.equal((await responses.find(r => r.status === 409)!.json()).error, "Nomor pertemuan sudah digunakan untuk kelompok ini.");
  assert.equal(sessions.length, 1); assert.equal(logs.length, 1);
});
