...
type Musyrif = { id: string; name: string; username: string; cityId: string | null };
...
export function GroupDetailClient({ group, musyrifs, canManage, canCreateMeeting, attendanceDashboard, isMusyrif = false, history = false }: { isMusyrif?: boolean; history?: boolean; attendanceDashboard?: React.ReactNode; group: { id: string; name: string; status: "ACTIVE" | "INACTIVE" | "DELETED"; sector: { name: string; mahalli: { name: string; city: { name: string } } }; attendanceSessions: Meeting[]; members: Member[]; assignments: { endedAt: Date | null; musyrif: { id: string; name: string; username: string } }[]; assignmentHistory: HistoryItem[] }; musyrifs: Musyrif[]; canManage: boolean; canCreateMeeting: boolean }) {
  ...
  const currentMusyrif = group.assignments.find(assignment => assignment.endedAt === null)?.musyrif;
  return <div>
    ...
    {isMusyrif && <nav className="mb-5 flex gap-4 text-sm"><Link href={`/dashboard/groups/${group.id}`}>Penugasan aktif</Link><Link href={`/dashboard/groups/${group.id}?history=1`}>Riwayat penugasan saya (hanya baca)</Link></nav>}
    <Alert message={error} />
    <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
      <div className="space-y-6">
        ...
        <Card className="overflow-hidden"><div className="flex flex-col justify-between gap-3 border-b border-line px-5 py-4 sm:flex-row sm:items-center"><div className="flex items-center gap-3"><div className="rounded-xl bg-brand-soft p-2.5 text-brand"><UsersRound size={19} /></div><div><h2 className="font-bold">Daftar anggota</h2><p className="mt-1 text-xs text-muted">{activeMembers.length} anggota aktif</p></div></div><button className="btn-quiet" type="button" onClick={() => setShowInactive((value) => !value)}>{showInactive ? "Sembunyikan nonaktif" : "Tampilkan nonaktif"}</button></div>{canManage && <div className="border-b border-line p-5"><MemberForm groupId={group.id} /></div>}<div className="divide-y divide-line">{members.map((member, index) => <div className="flex items-center justify-between px-5 py-4" key={member.id}><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-full bg-slate-100 text-xs font-bold text-slate-500">{String(index + 1).padStart(2, "0")}</div><div><p className={`text-sm font-semibold ${!member.isActive ? "text-slate-400 line-through" : ""}`}>{member.name}</p><p className="mt-0.5 text-[11px] text-muted">Bergabung {new Intl.DateTimeFormat("id-ID", { dateStyle: "medium" }).format(new Date(member.createdAt))}</p><Link href={`/dashboard/groups/${group.id}/members/${member.id}${history ? "?history=1" : ""}`} className="mt-2 inline-flex text-xs font-semibold text-brand">Riwayat presensi</Link></div></div>{canManage && member.isActive && <button className="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600" onClick={() => deactivate(member.id)} aria-label="Nonaktifkan anggota"><MoreHorizontal size={17} /></button>}</div>)}{!members.length && <div className="p-10 text-center text-sm text-muted">Belum ada anggota aktif.</div>}</div></Card>
...
function MeetingForm({ groupId }: { groupId: string }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(`/api/groups/${groupId}/sessions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ date, notes }) });
