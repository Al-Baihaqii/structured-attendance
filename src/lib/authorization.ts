...
export function canCreateUserRole(currentUser: SessionUser, targetRole: Role, targetScope: {
  cityId?: string | null;
  ...
  if (currentUser.role === "CITY_ADMIN") {
    return ["MAHALLI_ADMIN", "SECTOR_ADMIN", "MUSYRIF"].includes(targetRole) && targetScope.cityId === currentUser.cityId;
  }
  if (currentUser.role === "MAHALLI_ADMIN") {
    return ["SECTOR_ADMIN", "MUSYRIF"].includes(targetRole) && targetScope.mahalliId === currentUser.mahalliId;
  }
...
export function canViewGroup(currentUser: SessionUser, group: GroupWithScope) {
  if (currentUser.role === "SUPER_ADMIN") return true;
  if (currentUser.role === "CITY_ADMIN") return Boolean(currentUser.cityId && group.sector?.mahalli?.cityId === currentUser.cityId);
  if (currentUser.role === "MAHALLI_ADMIN") return Boolean(currentUser.mahalliId && group.sector?.mahalliId === currentUser.mahalliId);
  if (currentUser.role === "SECTOR_ADMIN") return Boolean(currentUser.sectorId && group.sectorId === currentUser.sectorId);
  return false;
}
...
export function canManageMember(currentUser: SessionUser, group: GroupWithScope) {
  return canManageGroup(currentUser, group) || currentUser.role === "MUSYRIF";
}

export function getAccessibleGroupsWhere(currentUser: SessionUser) {
  if (currentUser.role === "SUPER_ADMIN") return {};
  if (currentUser.role === "CITY_ADMIN") return { sector: { mahalli: { cityId: currentUser.cityId ?? "__none__" } } };
  if (currentUser.role === "MAHALLI_ADMIN") return { sector: { mahalliId: currentUser.mahalliId ?? "__none__" } };
  if (currentUser.role === "SECTOR_ADMIN") return { sectorId: currentUser.sectorId ?? "__none__" };
  return { userGroups: { some: { userId: currentUser.userId } } };
}

export function assertGroupAccess(currentUser: SessionUser, group: GroupWithScope, action: "view" | "manage" = "view") {
