...
import { AuthorizationError, getAccessibleGroupsWhere } from "./authorization";
...
export async function getHierarchicalAttendanceReport(user: SessionUser, parameters: { cityId?: unknown; mahalliId?: unknown; sectorId?: unknown } & ReportDateParameters) {
  ...
  return prisma.$transaction(async tx => {
    ...
    const sessions = await tx.attendanceSession.findMany({ where: { group: groupWhere, date }, select: { id: true, groupId: true } });
    const aggregates = await tx.attendanceRecord.groupBy({ by: ["sessionId", "status"], where: { session: { group: groupWhere, date } }, _count: { _all: true } });
