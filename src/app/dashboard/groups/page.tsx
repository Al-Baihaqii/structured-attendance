...
export default async function GroupsPage() {
  ...
  const [groups, sectors] = await Promise.all([
    prisma.group.findMany({ where: { ...getAccessibleGroupsWhere(user), status: { not: "DELETED" } }, include: { sector: { include: { mahalli: { include: { city: true } } } }, members: { where: { isActive: true }, select: { id: true } }, assignments: { include: { musyrif: { select: { name: true } } } } }, orderBy: { name: "asc" } }),
