...
export default async function UsersPage() {
  ...
  if (!user) return null;
  const where = getAccessibleUsersWhere(user);
  ...
  const [users, cities, sectors] = await Promise.all([
    ...
    prisma.city.findMany({ where: { ...scope.city, isActive: true }, include: { mahallis: { where: { ...scope.mahalli, isActive: true }, orderBy: { name: "asc" } } }, orderBy: { name: "asc" } }),
    prisma.sector.findMany({ where: { ...scope.sector, isActive: true }, include: { mahalli: { include: { city: true } } }, orderBy: { name: "asc" } }),
  ]);
